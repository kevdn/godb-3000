...
// RecoverToCallback recovers the WAL and calls the callback for each operation.
// This is a convenience method for applying WAL records during recovery.
func (w *WAL) RecoverToCallback(
	onInsert func(key, value []byte) error,
	onDelete func(key []byte) error,
) error {
	redo, undo, err := w.Recover()
	if err != nil {
		return err
	}

	// First, undo uncommitted transactions (in reverse order)
	for i := len(undo) - 1; i >= 0; i-- {
		record := undo[i]
		if record.RecordType == RecordTypeInsert {
			// Undo insert/update:
			...
			if len(record.OldValue) > 0 {
				...
				if err := onInsert(record.Key, record.OldValue); err != nil {
					return fmt.Errorf("failed to undo update: %w", err)
				}
			} else {
				// INSERT: delete the key
				if err := onDelete(record.Key); err != nil {
					return fmt.Errorf("failed to undo insert: %w", err)
				}
			}
		} else if record.RecordType == RecordTypeDelete {
	...
	// Then, redo committed transactions
	for _, record := range redo {
		if record.RecordType == RecordTypeInsert {
			if err := onInsert(record.Key, record.Value); err != nil {
